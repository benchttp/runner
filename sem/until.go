...
// RunUntil launches a goroutine to ping url as soon as a thread is
...
func RunUntil(quit <-chan struct{}, concurrency int, url string, timeout time.Duration) <-chan request.Record {
	// sem is a semaphore to constrain access to at most n concurrent threads.
	sem := make(chan int, concurrency)
	rec := make(chan request.Record)
